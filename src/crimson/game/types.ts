...
export class GameState {
  ...
  constructor(init: {
    ...
    this.gammaRamp = init.gammaRamp ?? 1.0;
    this.sndFreqAdjustmentEnabled = init.sndFreqAdjustmentEnabled ?? true;
    this.menuGround = init.menuGround ?? null;
    this.menuGroundCamera = init.menuGroundCamera ?? null;
    this.menuSignLocked = init.menuSignLocked ?? false;
    this.statsMenuEasterEggRoll = init.statsMenuEasterEggRoll ?? -1;
    this.pauseBackground = init.pauseBackground ?? null;
    this.pendingNetworkSession = init.pendingNetworkSession ?? null;
    this.networkRuntime = init.networkRuntime ?? null;
    this.networkInLobby = init.networkInLobby ?? false;
    this.networkWaitingForPlayers = init.networkWaitingForPlayers ?? false;
    this.networkExpectedPlayers = init.networkExpectedPlayers ?? 1;
    this.networkConnectedPlayers = init.networkConnectedPlayers ?? 1;
    this.networkDesyncCount = init.networkDesyncCount ?? 0;
    this.networkResyncFailureCount = init.networkResyncFailureCount ?? 0;
    this.networkLastError = init.networkLastError ?? '';
    this.pendingQuestLevel = init.pendingQuestLevel ?? null;
    this.pendingHighScores = init.pendingHighScores ?? null;
    this.questOutcome = init.questOutcome ?? null;
    this.questFailRetryCount = init.questFailRetryCount ?? 0;
    this.terrainRegenerateRequested = init.terrainRegenerateRequested ?? false;
    this.survivalElapsedMs = init.survivalElapsedMs ?? 0.0;
    this.demoTrialElapsedMs = init.demoTrialElapsedMs ?? 0;
    this.quitRequested = init.quitRequested ?? false;
    this.screenFadeAlpha = init.screenFadeAlpha ?? 0.0;
    this.screenFadeRamp = init.screenFadeRamp ?? false;
    this.runtimeUpdatesPerFrame = init.runtimeUpdatesPerFrame ?? 0;
    this.inputStallCount = init.inputStallCount ?? 0;
    this.ticksAdvancedPerFrame = init.ticksAdvancedPerFrame ?? 0;
    this.simMs = init.simMs ?? 0.0;
    this.presentationPlanMs = init.presentationPlanMs ?? 0.0;
    this.presentationApplyMs = init.presentationApplyMs ?? 0.0;
  }
}
